...
object Remapper {
    ...
    private fun probeEnvironment() {
        ...
        val officialName = minecraftEntry.getOrNull("official")?.replace('/', '.')
        val intermediaryName = minecraftEntry.getOrNull("intermediary")?.replace('/', '.')
        val namedName = minecraftEntry.getOrNull("named")?.replace('/', '.')

        logger.info("Probing environment... (official: $officialName, intermediary: $intermediaryName)")

        try {
            Class.forName(officialName ?: throw ClassNotFoundException())
            this.environment = "official"
            logger.info("Official environment detected.")
        } catch (_: ClassNotFoundException) {
            try {
                Class.forName(intermediaryName ?: throw ClassNotFoundException())
                ...
                try {
                    Class.forName(namedName ?: throw ClassNotFoundException())
                    this.environment = "named"
                    logger.info("Named environment detected.")
                } catch (_: ClassNotFoundException) {
    ...
    private fun net.fabricmc.mappings.ClassEntry.getOrNull(namespace: String): String? = runCatching {
        get(namespace)
    }.getOrNull()
