...
object VanillaTranslationRecognizer {
    ...
    fun shouldPreventLoad(pack: ResourcePack): Boolean {
        return runCatching {
            val modResourcePackClass = Class.forName("net.fabricmc.fabric.api.resource.ModResourcePack")

            if (!modResourcePackClass.isInstance(pack)) {
                return@runCatching false
            }

            val metadata = modResourcePackClass.getMethod("getFabricModMetadata").invoke(pack)
            val id = metadata.javaClass.getMethod("getId").invoke(metadata)
