...
object JsNetworkUtil {
    ...
    fun movePlayerGround(onGround: Boolean) = network.send(PlayerMoveC2SPacket.OnGroundOnly(onGround))

    @JvmName("movePlayerPosition")
    fun movePlayerPosition(x: Double, y: Double, z: Double, onGround: Boolean) =
        network.send(PlayerMoveC2SPacket.PositionAndOnGround(x, y, z, onGround))
    ...
    fun movePlayerPositionAndLook(x: Double, y: Double, z: Double, yaw: Float, pitch: Float, onGround: Boolean) =
        network.send(PlayerMoveC2SPacket.Full(x, y, z, yaw, pitch, onGround))

    @JvmName("movePlayerLook")
    fun movePlayerLook(yaw: Float, pitch: Float, onGround: Boolean) =
        network.send(PlayerMoveC2SPacket.LookAndOnGround(yaw, pitch, onGround))
