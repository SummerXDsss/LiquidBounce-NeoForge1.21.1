...
object ModuleSneak : Module("Sneak", Category.MOVEMENT) {
    ...
    private object Vanilla : Choice("Vanilla") {
        ...
        val networkTick = handler<PlayerNetworkMovementTickEvent> {
            ...
            if (!networkSneaking) {
                network.send(ClientCommandC2SPacket(player, ClientCommandC2SPacket.Mode.PRESS_SHIFT_KEY))
            }
        }

        override fun disable() {
            if (networkSneaking) {
                network.send(ClientCommandC2SPacket(player, ClientCommandC2SPacket.Mode.RELEASE_SHIFT_KEY))
