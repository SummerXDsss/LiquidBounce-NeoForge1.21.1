        ...
        val attackHandler = handler<AttackEvent> { event ->
            ...
            if (enemy is LivingEntity && enemy.hurtTime <= hurtTime && chance >= (0..100).random() &&
                ...
                if (player.isSprinting) {
                    network.send(ClientCommandC2SPacket(player, ClientCommandC2SPacket.Mode.STOP_SPRINTING))
                }

                network.send(ClientCommandC2SPacket(player, ClientCommandC2SPacket.Mode.START_SPRINTING))
                network.send(ClientCommandC2SPacket(player, ClientCommandC2SPacket.Mode.STOP_SPRINTING))
                network.send(ClientCommandC2SPacket(player, ClientCommandC2SPacket.Mode.START_SPRINTING))
