        ...
        if (player.isUsingItem && event.state == EventState.PRE) {
            ...
            if (hand == Hand.MAIN_HAND) {
                untracked {
                    ...
                    network.send(PlayerInteractItemC2SPacket(Hand.OFF_HAND, 0, player.yaw, player.pitch))
                }
            } else if (hand == Hand.OFF_HAND) {
                // Switch slots (based on 1.8 grim switch noslow)
                untracked {
                    val slot = player.inventory.selectedSlot
                    network.send(UpdateSelectedSlotC2SPacket(slot % 8 + 1))
                    network.send(UpdateSelectedSlotC2SPacket(slot % 7 + 2))
                    network.send(UpdateSelectedSlotC2SPacket(slot))
                }
            }
        }
    }
