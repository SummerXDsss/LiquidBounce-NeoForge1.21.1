...
object HideAppearance : Listenable {
    ...
    fun wipeClient() = thread(name = "wipe-client") {
        ...
        // Delete JAR file
        liquidBounceModPaths().forEach { path ->
            runCatching {
                path.toFile().delete()
            }
        }
    ...
    private fun liquidBounceModPaths(): List<Path> {
        return runCatching {
            val modListClass = Class.forName("net.neoforged.fml.ModList")
            val modList = modListClass.getMethod("get").invoke(null)
            val modFileInfo = modListClass.getMethod("getModFileById", String::class.java)
                .invoke(modList, "liquidbounce") ?: return@runCatching emptyList()
            val modFile = modFileInfo.javaClass.getMethod("getFile").invoke(modFileInfo)
            val filePath = modFile.javaClass.getMethod("getFilePath").invoke(modFile) as? Path

            filePath?.let(::listOf).orEmpty()
