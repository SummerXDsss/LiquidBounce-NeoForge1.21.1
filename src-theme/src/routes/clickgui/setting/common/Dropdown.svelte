<script lang="ts">
    ...
    import {translateSettingValue} from "../../../../theme/localization";
...
<div class="dropdown" class:expanded on:click={() => (expanded = !expanded)}>
    <div class="head" bind:this={dropdownHead}>
        <span class="text">{name} &bull; {translateSettingValue(value, $spaceSeperatedNames)}</span>
    </div>
    ...
    {#if expanded}
        <div class="options">
            {#each options as o}
                ...
                >
                    {translateSettingValue(o, $spaceSeperatedNames)}
                </div>
            {/each}
        </div>
    {/if}
