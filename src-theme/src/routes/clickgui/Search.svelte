<script lang="ts">
    ...
    import {moduleMatchesQuery, translateModuleAliases, translateModuleName} from "../../theme/localization";
    ...
    function filterModules() {
        ...
        filteredModules = modules.filter((m) => moduleMatchesQuery(m, query, $spaceSeperatedNames));
    }
                ...
                {#each filteredModules as {name, enabled, aliases}, index (name)}
                    ...
                    >
                        <div class="module-name">
                            {translateModuleName(name, $spaceSeperatedNames)}
                        </div>
                        <div class="aliases">
                            {#if aliases.length > 0}
                                （别名：{translateModuleAliases(aliases, $spaceSeperatedNames).join("，")}）
                            {/if}
                        </div>
                    </div>
                {/each}
            {:else}
                <div class="placeholder">未找到功能</div>
            {/if}
        </div>
    {/if}
</div>
