<script lang="ts">
    ...
    function gameModeLabel(gameMode: string) {
        const normalized = capitalize(gameMode);
        return gameModeLabels[normalized] ?? normalized;
    }

    function difficultyLabel(difficulty: string) {
        const normalized = capitalize(difficulty);
        return difficultyLabels[normalized] ?? normalized;
    }
...
<Menu>
    <OptionBar>
        ...
        <MultiSelect title="游戏模式" options={["Survival", "Creative", "Adventure", "Spectator"]}
                     labels={gameModeLabels} bind:values={gameModes}/>
        <MultiSelect title="难度" options={["Peaceful", "Easy", "Normal", "Hard"]}
                     labels={difficultyLabels} bind:values={difficulties}/>
    </OptionBar>
        ...
        {#each renderedWorlds as world}
            <MenuListItem
                ...
                <svelte:fragment slot="tag">
                    <MenuListItemTag text={gameModeLabel(world.gameMode)}/>
                    <MenuListItemTag text={difficultyLabel(world.difficulty)}/>
                ...
                <svelte:fragment slot="always-visible">
                    <MenuListItemButton title="进入" icon="play" on:click={() => openWorld(world.name)}/>
                </svelte:fragment>
            </MenuListItem>
        {/each}
    </MenuList>
    ...
    <BottomButtonWrapper>
        <ButtonContainer>
            <IconTextButton icon="icon-plus-circle.svg" title="新建" on:click={() => openScreen("create_world")}/>
        </ButtonContainer>
